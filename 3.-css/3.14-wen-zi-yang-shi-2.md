...
* h：Hue 色相。\( 0 ~ 360\)
* s：Saturation 飽合度。\(0% ~ 100%\)，一個顏色若飽合度愈高，愈接近純色；一個顏色若飽合度低，就愈接近黑色。
...
在 `practice` 資料夾下，建立 `font.html` 來測試觀察。`body` 標籤內容如下：
...
以上述為例，瀏覽器會優先用「微軟正黑體」，如果電腦沒有這個字體，就繼續往後找，下一個是「BiauKai」，如果有，就使用這個字體。依此類推。
...
![sans-serif &#x7121;&#x896F;&#x7DDA;&#x9AD4;](../.gitbook/assets/sans_serif.png)



### 使用 Google Font 載入字體

[預先選好的字體\(Beth Ellen\)](https://fonts.google.com/specimen/Beth+Ellen?query=Beth+Ellen&preview.text_type=custom&selection.family=Beth+Ellen&sidebar.open)

![](../.gitbook/assets/google_font_embed.png)
...
* 字體的載入有些瀏覽器限定需相同網域\(同源政策\)。
* 如果要跨網域允許的話，伺服器需設定 **`Access-Control-Allow-Origin: *`** 。
